...
;;;;prepare for the border
	...
	(set! border-layer (car (gimp-layer-new image width height typeA "Border" 100 NORMAL-MODE)))
    (gimp-image-add-layer image border-layer -1)
	
	(set! tint-layer (car (gimp-layer-new image width height typeA "Tint" 100 NORMAL-MODE)))
	...
	(set! bump-layer (car (gimp-layer-new image width height typeA "Grain" 100 NORMAL-MODE)))
...
;;;;create the top of border
	...
	(gimp-edit-blend border-layer CUSTOM-MODE NORMAL-MODE  GRADIENT-LINEAR 100 0 REPEAT-NONE reverse FALSE 3 0.2 TRUE (/ width 2) 0 (/ width 2) ysize)

;;;;create the bottom of border
	(gimp-free-select image 10 *newpoint-bot* 2 TRUE FALSE 15)
	(gimp-edit-blend border-layer CUSTOM-MODE NORMAL-MODE  GRADIENT-LINEAR 100 0 REPEAT-NONE reverse FALSE 3 0.2 TRUE (/ width 2) height (/ width 2) (+ old-height ysize))

;;;;create the left of border
	(gimp-free-select image 10 *newpoint-left* 2 TRUE FALSE 15)
	(gimp-edit-blend border-layer CUSTOM-MODE NORMAL-MODE  GRADIENT-LINEAR 100 0 REPEAT-NONE reverse FALSE 3 0.2 TRUE 0 (/ height 2) xsize (/ height 2))

;;;;create the right of border
	(gimp-free-select image 10 *newpoint-right* 2 TRUE FALSE 15)
	(gimp-edit-blend border-layer CUSTOM-MODE NORMAL-MODE  GRADIENT-LINEAR 100 0 REPEAT-NONE reverse FALSE 3 0.2 TRUE width (/ height 2) (+ old-width xsize) (/ height 2))
    (gimp-selection-none image)
...
;;;;apply the vignette
	...
	(gimp-edit-blend vignette-layer CUSTOM-MODE NORMAL-MODE   GRADIENT-RADIAL 100 0 REPEAT-NONE TRUE FALSE 3 0.2 TRUE (/ vig-x 2) (/ vig-y 2) vig-x vig-y)
	(gimp-selection-none image)
